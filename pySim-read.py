...
if __name__ == '__main__':
	...
	# Select USIM application by its AID
	if '9000' == card.select_adf_by_aid():
		#EF.ePDGIdEm - Emergency ePDG Identifier
		try:
			(res, sw) = card.read_binary('6ff5')
			if sw == '9000':
				content = dec_addr_tlv(res)
				print("ePDGIdEm: \n %s" % (len(content) and content or '\tNot available\n',))
			else:
				print("ePDGIdEm: Can't read, response code = %s" % (sw,))
		except Exception as e:
	...
	if '9000' == card.select_adf_by_aid():
		...
		try:
			(res, sw) = card._scc.read_binary(['6ff4'])
			if sw == '9000':
	...
	if '9000' == card.select_adf_by_aid(adf="isim"):
		...
		try:
			rec_cnt = card._scc.record_count(['6f09'])
			print("P-CSCF:")
			pcscf_recs = ""
			for i in range(0, rec_cnt):
				(res, sw) = card._scc.read_record(['6f09'], i + 1)
				if sw == '9000':
					content = dec_addr_tlv(res)
	...
	if '9000' == card.select_adf_by_aid(adf="isim"):
		...
		try:
			(res, sw) = card._scc.read_binary(['6f02'])
			if sw == '9000':
				# Skip the inital tag value ('80') byte and get length of contents
				length = int(res[2:4], 16)
				content = h2s(res[4:4+(length*2)])
				print("IMS private user identity: %s" % (len(content) and content or 'Not available',))
			else:
	...
	if '9000' == card.select_adf_by_aid(adf="isim"):
		...
		try:
			rec_cnt = card._scc.record_count(['6f04'])
			print("IMS public user identity:")
			impu_recs = ""
			for i in range(0, rec_cnt):
				(res, sw) = card._scc.read_record(['6f04'], i + 1)
				if sw == '9000':
					# Skip the inital tag value ('80') byte and get length of contents
					length = int(res[2:4], 16)
					content = h2s(res[4:4+(length*2)])
	...
	if '9000' == card.select_adf_by_aid(adf="isim"):
		...
		try:
			rec_cnt = card._scc.record_count(['6fe7'])
			print("UICC IARI:")
			uiari_recs = ""
			for i in range(0, rec_cnt):
				(res, sw) = card._scc.read_record(['6fe7'], i + 1)
				if sw == '9000':
					# Skip the inital tag value ('80') byte and get length of contents
					length = int(res[2:4], 16)
					content = h2s(res[4:4+(length*2)])
